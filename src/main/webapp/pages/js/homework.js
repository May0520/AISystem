...
$(document).ready(function(){
    ...
    $('#submit').click(function(){
        ...
        var deadline = $('#reservation').val()
        var content = deadline+"\n"+$('#editor-one').html()
        result = $.ajax({
            url: "http://"+addr+"/SaveFile",
            async: false,
            data: {
                dir: "/user/aiSystemUser/"+username+"/homework/"+course+"/",
                title:title,
                deadline:deadline,
                content:content
            },
...
function getHomeworkList(course) {
    var dir = "/user/aiSystemUser/"+username+"/homework/"+course+"/";
    htmlobj = $.ajax({
        url: "http://"+addr+"/GetHomeworkList",
        async: false,
        data: {
            dir: dir
        },
    ...
    $.each($.parseJSON(htmlobj.responseText), function (idx, obj) {
        ...
        str+='<tr> <td>'+num+'</td>'+
            ...
            '          </div>'+
            '          <div class="form-group">'+
            '            <label class="control-label col-md-3 col-sm-3 col-xs-12">作业提交日期</label>'+
            '            <div class="controls col-md-9 col-sm-9 col-xs-12">'+
            '              <div class="input-prepend input-group">'+
            '                <span class="add-on input-group-addon"><i class="glyphicon glyphicon-calendar fa fa-calendar"></i></span>'+
            '                <input type="text" style="width: 200px" name="reservation" id="dateRange'+num+'" class="form-control" value="'+deadline+'" />'+
            '              </div>'+
            '            </div>'+
            '          </div>'+
            '            <div class="form-group">'+
...
function changeHomework(obj){
    ...
    var deadline = $('#dateRange'+num).val()
    var content = $('#editor-text-'+num).html()
    result = $.ajax({
        url: "http://"+addr+"/ChangeHomework",
        async: false,
        data: {
            dir: "/user/aiSystemUser/"+username+"/homework/"+course+"/",
            old_title:old_title,
...
function editHomework(obj){
    ...
    var deadline = td.eq(3).text().trim();
    var path="/user/aiSystemUser/"+username+"/homework/"+$('#course').html()+"/"+name+".txt"
    result = $.ajax({
        url: "http://"+addr+"/GetHomeworkContent",
        async: false,
        data: {
            path: path
        },
